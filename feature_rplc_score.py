...
def feature_rplc_on_model(x, y, model, random_state):
    ...
    for i in range(x.shape[1]):
        ...
        x_ = x.copy()
        x_[:, i] = np.random.normal(size=(1, x.shape[0]))
...
def feature_rplc_score(data: dict,
    ...
    for model_idx, model_name in enumerate(model_names):
        ...
        for data_idx, (data_name, x) in enumerate(data.items()):
            ...
            scores = feature_rplc_on_model(x, target, model, random_state)
    ...
    plt.savefig('./img/feature_rplc_score.png')
    if show_img:
        plt.show()
